...
const parseMermaidFlowchart = (mermaidText: string): MermaidParseResult => {
  ...
  lines.forEach(line => {
    // Handle node definitions with labels - improved regex for better parsing
    ...
    if (nodeDefMatch) {
      ...
      if (nodeId && label) {
        ...
        if (!nodes.find(n => n.id === nodeId)) {
          ...
          // Clean up the label by removing the act prefix
    ...
    // Handle connections with labels - improved regex for more mermaid syntax including edge labels
    const connectionMatch = line.match(/(\w+)\s*(-->|---|\.-\.->|\.-\.-|==>|==|->|-\.->|<-->|<->|-.->|\.\.\.|===|~~~)\s*\|?([^|]*)\|?\s*(\w+)/);
    if (connectionMatch) {
      const [, source, connector, edgeLabel, target] = connectionMatch;
